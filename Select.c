 ...
 *
 */
Selector selector_create(float x, float y, float w, float h, char **items, int item_num, int default_selection,
                         ALLEGRO_COLOR color) {
    ...
    selector.font = mainFont;
    selector.rounded = 2;
    selector.thickness = 4;

    selector.event_register = &event_register;
...
void drawSelector(Selector *selector) {
    ...
    al_draw_text(selector->font, selector->color, cx, cy - al_get_font_ascent(selector->font) / 2 , ALLEGRO_ALIGN_CENTRE,
                 text_to_show);
    ...
    al_draw_filled_triangle(cx, selector->btn_body[0].y,
                            ...
                            selector->btn_body[0].x + selector->btn_body[0].w,
                            selector->btn_body[0].y + selector->btn_body[0].h,
                            color_now[0]);
...
void checkSelectorHovered(Selector *selector, int x, int y) {
    selector->hovered[0] = pnt_in_rect(x, y, selector->btn_body[0]);
    selector->hovered[1] = pnt_in_rect(x, y, selector->btn_body[1]);
}
...
static void event_register(Selector *selector, ALLEGRO_EVENT event) {
    if (event.type == ALLEGRO_EVENT_MOUSE_AXES) {
        checkSelectorHovered(selector, event.mouse.x, event.mouse.y);
    } else if (event.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN) {
        selectorOnMouseDown(selector);
    }
}
